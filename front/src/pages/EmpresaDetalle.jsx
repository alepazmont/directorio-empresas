  ...
  return (
    <div className="empresa-detalle">
      ...
      <div className="empresa-datos">
        ...
        <div className="info-empresa">
          ...
          {empresa.listaProd && empresa.listaProd.length > 0 && (
            <p><strong>Productos:</strong> {empresa.listaProd.length > 1 ? empresa.listaProd.join(', ') : empresa.listaProd[0]}</p>
          )}
          {empresa.listaServ && empresa.listaServ.length > 0 && (
            <p><strong>Servicios:</strong> {empresa.listaServ.length > 1 ? empresa.listaServ.join(', ') : empresa.listaServ[0]}</p>
          )}
          <p><strong>Dirección:</strong> {empresa.direccion}, {empresa.codigoPostal}</p>
          <p><strong>Parada de Metro:</strong> {empresa.paradaMetro}</p>
          {empresa.telefono && empresa.telefono.length > 0 && (
            <p><strong>Teléfono:</strong> {empresa.telefono.length > 1 ? empresa.telefono.join(', ') : empresa.telefono[0]}</p>
          )}
            ...
            {empresa.redes && empresa.redes.length > 0 && (
              ...
              <ul>
                {empresa.redes.map((red, index) => (                    
                 <li key={index}><a key={index} href={red} target="_blank" rel="noopener noreferrer">{red}</a></li>
                ))}
                </ul>
              </div>
            )}
      ...
      <div className="galeria-fotos">
        ...
        <div className="fotos">
          {empresa.galeriaFotos && empresa.galeriaFotos.length > 0 && (
